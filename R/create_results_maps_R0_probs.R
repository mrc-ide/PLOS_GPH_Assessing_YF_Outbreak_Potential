...
for(data_select in 1:length(data_types)){
  ...
  values_sa=epi_data_sa[,1+data_select]
  
  create_map(shape_data_af,values_af,scale=scale,colour_scale,pixels_max=1440,text_size=2,map_title="",
             legend_title=data_types[data_select],legend_position="bottomleft",legend_format="e",legend_dp=1,
  ...
  create_map(shape_data_sa,values_sa,scale=scale,colour_scale,pixels_max=1440,text_size=2,map_title="",
             ...
             output_file=paste0(comp,"/",results_folder,"/R0_probs_map_sa",data_select,".png",sep=""))
}
