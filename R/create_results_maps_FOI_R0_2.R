...
{
  epi_data=readRDS(paste(comp,data_folder,"FOI_R0_summary_734regions_alt_no_Brazil_multiplier_new.Rds",sep="/")) 
...

{
  ...
  for(i in 1:length(country_list_sa)){
    ...
    shapefiles_sa_countries[i]=paste(comp,"/",shapefile_folder,"/",country_list_sa[i],"/gadm36_",country_list_sa[i],"_0.shp",
                                     sep="")
  }
...
create_map(shape_data_af,FOI_values_af,scale=scale_FOI,colour_scale,pixels_max=1440,text_size=1.5,
           ...
           output_file=paste(comp,results_folder,"FOI_map_af.png",sep="/"))
create_map(shape_data_sa,FOI_values_sa,scale=scale_FOI,colour_scale,pixels_max=1440,text_size=1.5,
           ...
           output_file=paste(comp,results_folder,"/FOI_map_sa.png",sep="/"))
