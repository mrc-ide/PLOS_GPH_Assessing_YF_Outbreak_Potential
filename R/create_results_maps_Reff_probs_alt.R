...
results_folder=paste0("Documents/0 - Publications+Reports/Paper 07 - Yellow fever model paper revision/",
                     "images/Real - main text/Reff prob maps vacceff 0975/")
...
enviro_glm=read.csv(file=paste(comp,data_folder2,"enviro_glm_6covs.csv",sep="/"),header=TRUE)
{
  ...
  vacc_eff_values=chain_data$vaccine_efficacy
...
for(coverage_threshold in c(0.5,0.6,0.8)){
  ...
  {
    #Get R_eff values based on vaccine coverage in 1-60 year olds based on reported vaccination effectiveness and
    #test if >=0.5, 0.7, 1.0
    ...
    for(n_region in 1:n_regions){
      ...
      #R_eff_values=R0_values*(1-(coverage_threshold*pop_ratio[n_region]*vacc_eff_values))
      R_eff_values=R0_values*(1-(coverage_threshold*pop_ratio[n_region]*0.975))
      prob_data$`P(Reff>=0.5)`[n_region]=length(R_eff_values[R_eff_values>=0.5])
      prob_data$`P(Reff>=0.7)`[n_region]=length(R_eff_values[R_eff_values>=0.7])
      prob_data$`P(Reff>=1.0)`[n_region]=length(R_eff_values[R_eff_values>=1.0])
    }
